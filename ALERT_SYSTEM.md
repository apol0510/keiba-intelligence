...
1. 過去7日間の日付をループ
   ↓
2. keiba-data-sharedに予想データがあるかチェック
   ↓
3. keiba-data-sharedに結果データがあるかチェック
   ↓
4. アーカイブに反映されているかチェック
   ↓
5. 不一致があればアラートメール送信
```
...
**アラート条件**:
1. **予想あり + 結果なし** → 🚨🚨 結果データ未登録アラート（最優先）
2. **予想あり + 結果あり + アーカイブなし** → 🚨 アーカイブ未反映アラート
...
| 種別 | 件名 |
|------|------|
| **JRA結果未登録** | **🚨🚨 JRA結果データ未登録検出 - keiba-data-shared** |
| **南関結果未登録** | **🚨🚨 南関結果データ未登録検出 - keiba-data-shared** |
...
JRA予想データは存在しますが、結果データがkeiba-data-sharedに登録されていません。
