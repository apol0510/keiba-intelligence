...
| R-3 | 無料会員の印は **「印」1 列に複数を重複付与**する。◎3〜5 / ○3〜5 / ▲3〜5 / △約10 頭を重ね、該当しない馬は空欄。**評価順 1 位と 2 位は必ず同じ印の組み合わせ**にして本命を一意に特定させない | free |
...
#### R-3 の詳細（無料会員の印・2026-08-29 改訂）

「印」は **1 列だけ**。評価項目ごとの列は作らない。1 頭に複数の印を重ねて付ける。

各印は KI 評価順の帯（バンド）として重なる。△＝10 頭のレースの例:
...
- **◎ は 3 頭以上、○ と ▲ は評価順 3 位以降から始まる**。
  したがって **評価順 1 位と 2 位の印は必ず一致**し、印から本命を一意に特定できない。
- 頭数が少ないレースでは △ が「出走頭数 − 2」まで縮む（空欄を必ず残すため）。
  12 頭以上なら △＝10 頭。8 頭以上なら ◎○▲ は 3〜5 頭に収まる。
- **ランダム・ダミーは使わない。** 印は既存の KI 評価
  （役割の順序 → pt 降順 → 馬番昇順）から決定論的に算出する。
- **有料 tier では印を出さない**（R-8）。有料は AI指数の実数値・AI結論・買い目で結論を示すため、印は重複する。
