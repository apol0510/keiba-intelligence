...
| 項目 | 内容 |
|---|---|
| 点数 / 購入額 | 抑えを除いた実点数と、`点数 × 100 円` |
| 組み合わせ | 1 点 ＝ 1 チップ（`4 → 6`）。順序付きの組をすべて並べる |
| 軸ごとの内訳 | `軸 4 → 6・8・12・3・11・9` |
| 抑え | 別枠。**買い目には含まれない**ことを明記 |
| 注記 | 「1点100円で計算」「回収率の投資基準（全レース5点固定）とは別の数値」 |
...
| レース | 買い目 | 点数 | 購入額 |
|---|---|---|---|
| 通常 | `4-6.8.12.3.11.9(抑え10.7.5)` ＋ `6-4.8.12.3.11.9(抑え10.7.5)` | **16点** | ¥1,600 |
| メイン | `3-5.7.8.10.12` | **5点** | ¥500 |

🔴 **点数の意味に注意。** ここで数えるのは **表示した買い目の実点数**であり、
`archiveResults` の回収率が使う投資基準（**全レース 5 点固定**）とは **別概念**である
（`BET_POINT_LOGIC.md`「出力フィールド」の注記）。画面にもその旨を出している。
