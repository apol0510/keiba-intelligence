...
| R-9 | **並べ替え（AI指数順）・抽出（買い目の馬だけ）のボタンは有料 tier のみ**。free/guest には HTML ごと出さない（並びから評価順位が読めるため。§4.4） | light+ |
...
### 4.4 並べ替え・抽出ボタン（🔴 有料 tier 限定・2026-08-30 追加）

出馬表の上に 2 つのボタンを置く。**`view.showBetting` が true のときだけ描画する。**

| ボタン | 動作 | 押した後の表示 |
|---|---|---|
| **⇅ AI指数順** | AI指数の高い順に並べ替える（指数が無い馬は末尾） | 「馬番順に戻す」 |
| **◎ 買い目の馬だけ** | 買い目に出てくる馬だけを残す | 「全頭に戻す」 |

2 つは併用できる（「AI指数の高い順 ／ 買い目の7頭のみ」と状態を横に表示する）。
...
free/guest の HTML には **ボタンも `data-pt` も `data-bet` も存在しない**。

| 出力 | guest / free | light / premium |
|---|---|---|
| `.ret-tools`（ボタン） | ❌ 出さない | ✅ |
| `data-pt`（AI指数の実数値） | ❌ 空文字 | ✅ |
| `data-bet`（買い目に居るか） | ❌ 空文字 | ✅ |
| `betHorses` prop | ❌ 空配列 | ✅ |
...
| 目的 | ファイル |
|---|---|
| 買い目の文字列 → 馬番（**表示専用**） | `src/utils/bettingLineHorses.js` |
| ボタン・行データ・並べ替え/抽出の JS | `src/components/newspaper/RaceEntryTable.astro` |
| tier で `betHorses` を絞る | `src/components/newspaper/RaceNewspaper.astro` |

🔴 `bettingLineHorses.js` を **的中判定に使わない**。
的中判定の単一源は `umatanHit.js` の `checkUmatanHit` であり、軸・本線相手・方向（F3）を
厳密に扱う。`bettingLineHorses.js` は **画面に印字されている馬番を拾うだけ**で、
抑えも含める（画面に出ているため）。ここを流用すると抑えを買ったことになる。

並べ替えは **詳細アコーディオンの行を本体行と一緒に動かす**。
抽出で隠した行は、開いていた詳細も閉じる。

テストで固定: `bettingLineHorses.test.mjs`
