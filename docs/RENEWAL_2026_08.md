...
| 色 | 役割 | 適用先 | トークン |
|---|---|---|---|
| **ブルー** | ナビゲーション（どこを見ているか） | 選択中のレースタブ・会場タブ、レース展望のタグ | `--primary-*` |
| **パープル** | AI の判断 | 結論セクション（背景・タグ） | `--conclusion-*` |
| **オレンジ** | 行動（お金が動く） | 「買い目を抽出」ON、馬単バッジ、推奨点数・購入額、1着の馬番、発走時刻 | `--secondary-*` |
| **スレート** | 道具（中立の操作）・分類 | 「AI指数順」ON、会場の分類ラベル（南関東 / 中央競馬） | `--tool-*` |

読み方が一本の線になる:

```
青（今どこ）→ 紫（AIはこう判断した）→ 橙（だから、こう買う）
             スレートは道具なので主張しない
```

- 会場ラベル（南関東 / 中央競馬）は **ブルーをレースタブに譲って**スレートにした。
  分類は「現在位置」ではないため。
- 買い目パネルの背景も淡いオレンジ寄りにし、チップの枠線をオレンジ系に合わせた。
- 🔴 テストで固定: `entitlementRoutes.test.mjs`
  「配色の役割分担」— 買い目まわりにブルーが戻ると落ちる。
