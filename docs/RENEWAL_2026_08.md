...
青 → 紫 → 桃 → 橙 の **1 本のスペクトラム**を、画面を読む順に割り当てる。

| グラデーション | 役割 | 適用先 | トークン |
|---|---|---|---|
| **青 → 紫**<br>`#2563eb → #7c3aed` | いまどこを見ているか | 選択中のレースタブ、会場タブの文字 | `--grad-nav` |
...
| スレート<br>`#334155 → #475569` | 道具・分類（主張しない） | 「AI指数順」ON、会場の分類ラベル、抑えのタグ | `--tool-gradient` |

読み方が一本の線になる:

```
青→紫（今どこ） → 紫→桃（AIはこう判断した） → 桃→橙（だから、こう買う）
                  スレートは道具なので主張しない
```
...
- 🔴 テストで固定: `entitlementRoutes.test.mjs`「配色の役割分担」
  - 3 つのグラデーションが **2 色**であること（同系色の濃淡に戻ると落ちる）
