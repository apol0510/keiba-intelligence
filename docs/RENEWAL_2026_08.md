...
| R-3 | 無料会員の印は **「印」1 列に複数を重複付与**する。◎3〜5 / ○3〜5 / ▲3〜5 / △約10 頭を重ね、該当しない馬は空欄。**本命は分かってよい**（○ を評価順 2 位から始め、最上位だけが「◎△」の一意の組み合わせになる）。守るのは**相手が誰か**であり、△ を買い目の相手（5〜6 頭）より広く取ることで担保する | free |
...
- **本命は分かってよい。** ○ を評価順 2 位から始めることで、
  評価最上位だけが「◎△」という一意の組み合わせになる。
