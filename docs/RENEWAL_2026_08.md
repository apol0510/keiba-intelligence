...
| # | 論点 | 確定内容 |
|---|---|---|
| U-1 | 無料開放の深さ | **未登録: 印と買い目以外はすべて見せる** / **無料会員: 印を見せる**（🔴 漏洩が成立するレースだけ △ を出さない。R-3）/ **有料: 買い目を見せる** |
...
| R-3 | 無料会員の印は **「印」1 列に、指数ごとの印を合算**する（新聞の総合印）。**指数 1 本 ＝ 記者 1 人**とみなし、各軸が 1 位◎ / 2 位○ / 3 位▲ / 4〜7 位△ を出す。同じ記号は重なる（'◎◎○▲'）。**本命は分かってよい**（複数の指数が一致すれば自然に印が集まる）。守るのは**相手が誰か**であり、△ を買い目の相手（5〜6 頭）より広く取ることで担保する。🔴 **少頭数では生成側で担保できない**ため、△ 集合が買い目 1 行の相手集合と完全一致するレースだけ **△ を描画しない**（◎○▲ はそのまま出す。2026-09-07 改訂・下の「担保できない少頭数」参照） | free |
...
- **相手は絞り込ませない。** △ の集合が買い目の相手の集合と一致しないようにする。
  ...
  🔴 少頭数では生成側で一致を避けられないため、**表示側で △ を落として担保する**
  （「担保できない少頭数がある」参照）。
...
（生成仕様は `attentionMarks.test.mjs`、実際に描画される印の漏洩検証は
`attentionMarkPolicy.test.mjs` で固定している）。

🔴 **この値では漏洩を完全には防げない**（2026-09-07 判明）。上の突き合わせは
南関 12 レース・当時のデータでのみ 0 件だったにすぎない。次節を参照。

##### 🔴 担保できない少頭数がある（2026-09-07 改訂）

R-3 は「△ を相手より広く取る」ことで相手を守る設計だが、**少頭数では算術的に成立しない**。

8 頭立てはプールが 7 頭（空欄 1 頭）、買い目の相手が 6 頭である。
△ が相手より真に広くなるにはプール全 7 頭に △ が付くしかなく、
それは指数の並び次第で成立しない（◎○▲ だけを集めた馬が出ると △ は 6 頭に留まる）。

保存済み予想 236 ファイル・1,931 レース（8 頭以上・買い目あり）を突き合わせた実測:

| 出走頭数 | △ 集合が買い目 1 行の相手と完全一致 |
|---|---|
| 8 頭 | 16 / 87（**18.4%**） |
| 9 頭 | 9 / 192（4.7%） |
| 10 頭 | 6 / 269（2.2%） |
...
**決定**: 生成ロジック（`assignFreeMarks`）は変えない。
**表示側**で、完全一致が成立するレースだけ **△ を描画しない**（`noDown`）。

- ◎○▲ は残す。R-3 が「**本命は分かってよい**」として公開を許容している情報であり、
  守る対象（相手が誰か）を担っているのは △ だけであるため。
- 判定単位は **買い目 1 行**（R-3 の「軸 → 相手 5〜6 頭」）。
  複数行を union すると実際より広い集合になり、どの買い目が復元できるかを測れない。
- 判定は **server-side**（`RaceNewspaper.astro` の frontmatter。全予想ページが `prerender = false`）。
  **買い目そのものは free 向けの props にも HTML にも載せない。** 外へ出すのは `'full' | 'noDown'` だけ。
- 上の 38 レースは適用後 **0 件**（`noDown` 38 レースで抑止）。

> **残留リスク（承認済み・2026-09-07）**: △ が出ないこと自体が
> 「このレースは一致が成立した」というシグナルになる。ただし △ 集合そのものは復元できず、
> 8 頭立ての例では相手集合の候補は 3 通り残る（印を全部消せば 28 通り）。
> 印を全部消す案は U-1「無料会員は印を見せる」を毀損するため採らない。
> シグナル自体を消すには「9 頭以下は常に △ を出さない」等の一律適用が必要だが、
> 対象を漏洩レースに限定する方針のため採用しない。
...
🔴 空欄を 1 頭に減らしても 8 頭立ての一致は残る（2026-09-07 実測 18.4%）。
最終的な担保は表示側のポリシーが行う。
...
- 印の表示ポリシー … `src/utils/attentionMarkPolicy.js`
  （`markPolicyFor` が `'full' | 'noDown'` を返し、`applyMarkPolicy` が △ を落とす。
  判定は `RaceNewspaper.astro`、適用は `RaceEntryTable.astro`。
  🔴 `RaceEntryTable` には買い目を渡さない）
...
| 無料会員 | `free` | メール登録＋マジックリンク認証 | 上記 ＋ **印（◎○▲△ を 1 頭に複数付与・該当しない馬は空欄。🔴 漏洩が成立するレースは △ が出ず ◎○▲ だけになる。R-3）** ＋ **AI指数の列（モザイク・値なし）** ／ 結論なし・買い目なし |
...
- 🔴 **漏洩が成立するレースは △ を描画しない**（`markPolicy = 'noDown'`）。◎○▲ は出す。列自体は残す（R-3）。
  判定は `RaceNewspaper.astro` が server-side で行い、`RaceEntryTable` へは `'full' | 'noDown'` だけを渡す。
...
- [x] 🔴 漏洩が成立するレースで **△ が出ず ◎○▲ は出る**ことを、回帰ケース（2026-09-08 川崎 1R / 8R / 12R）と
      実データ走査で固定（`attentionMarkPolicy.test.mjs`。保存済み 1,931 レースで適用後の一致 **0 件**）
