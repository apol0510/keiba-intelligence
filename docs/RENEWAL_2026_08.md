...
| R-3 | 無料会員の印は **序列を持たない**。◎○▲△ をやめ、**2〜5 頭**に**同一種類の「注目」印**だけを付ける | free |
...
- 注目印の選定 … `src/utils/attentionMarks.js`（`attentionHorseNumbers` は **Set** を返し順序を持たない）
...
| 無料会員 | `free` | メール登録＋マジックリンク認証 | 上記 ＋ **注目印（2〜5 頭・同一種類・序列なし）** ／ AI指数なし・結論なし・買い目なし |
| 有料（ライト） | `light` | Stripe サブスク（下位） | 上記 ＋ **◎○▲△ の印**・**AI指数**・**AI結論**・**買い目**（対象会場・レース範囲は §6） |
...
- **注目印の列は `showMarks`（free 以上）、◎○▲△ と AI指数の列は `showBetting`（有料）が
  false なら列ごと出さない**（CSS で隠さない）。
