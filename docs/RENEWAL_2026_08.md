...
| R-3 | 無料会員の印は **「印」1 列に、指数ごとの印を合算**する（新聞の総合印）。**指数 1 本 ＝ 記者 1 人**とみなし、各軸が 1 位◎ / 2 位○ / 3 位▲ / 4〜10 位△ を出す。同じ記号は重なる（'◎◎○▲'）。**本命は分かってよい**（複数の指数が一致すれば自然に印が集まる）。守るのは**相手が誰か**であり、△ を買い目の相手（5〜6 頭）より広く取ることで担保する | free |
...
- **相手は絞り込ませない。** △ を **買い目の相手より広く**取る。
  各軸が上位 10 頭まで印を出すため、合算後の △ は 8〜12 頭になる。
  ◎○▲ を明示しても、正解の 5〜6 頭は △ に埋もれて特定できない。
- 無料で出せるものは惜しまず出す。判断材料（馬柱・過去走・AI短評・展開予想・印）は
  すべて公開し、**結論の組み立て**（相手の確定リスト・順序・点数）だけを有料に残す。

🔴 **各軸の △ は「上位 10 頭まで」で固定する。軸の本数で変えない。**
指数どうしは順位が似るため、△ を狭くすると合算しても △ が 4〜6 頭にしかならず、
買い目の相手を絞り込めてしまう（2026-08-29 に実データで確認し、修正した）。
...
- 印の算出 … `src/utils/attentionMarks.js`
  ...
  `assignFreeMarks` で合算。Math.random / Date に依存しない）
