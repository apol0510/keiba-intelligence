...
└──────────────────────────────────────┘
┌ 馬柱カード（出走頭数分） ──────────────────┐
│ [枠色][馬番] 馬名          印(tier: free+)  PT(free+) │
│ 父(母父) / 牡5 / 斤量56 / 騎手 / 厩舎 / 馬体重±       │
│ 【AI短評】← 文章 1〜3 文                            │
│ 特徴量ミニバー ×6                                   │
│ 前3走の圧縮馬柱（着順・場・距離・時計・上3F・通過・人気）│
└──────────────────────────────────────┘
┌ 結論（買い目: light+ のみ） ────────────────┐
│ 【AI結論】← 文章 ／ 馬単買い目                       │
└──────────────────────────────────────┘
...
| 過去走の縦積み | 直近 3 走を圧縮ブロックで表示（4 走目以降は折りたたみ） |
...
出走馬は縦スクロールのカード列とする。紙面の横方向マトリクスは**再現しない**
（スマートフォンで読めないため）。情報密度は「1 頭 1 カード」に圧縮して担保する。
