...
| R-5 | **AI指数（pt）は有料 tier のみ**。数値があれば順位が完全に復元できるため | light+ |
...
- 有料 tier は従来どおり **評価を絞った単一の印**（役割由来）と AI指数を出す。
...
| 無料会員 | `free` | メール登録＋マジックリンク認証 | 上記 ＋ **印（◎○▲△ を 1 頭に複数付与・該当しない馬は空欄）** ／ AI指数なし・結論なし・買い目なし |
| 有料（ライト） | `light` | Stripe サブスク（下位） | 上記 ＋ **評価を絞った単一の印**・**AI指数**・**AI結論**・**買い目**（対象会場・レース範囲は §6） |
...
- **印の列は `showMarks`（free 以上）、AI指数の列は `showBetting`（有料）が
  false なら列ごと出さない**（CSS で隠さない）。印の列は **1 列だけ**。
