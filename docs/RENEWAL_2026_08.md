...
| 組み合わせ | 1 点 ＝ 1 チップ（`4 → 6`）。**推奨点数ぶんだけ**並べる（見出しと数が一致） |
...
- 表示する組み合わせも推奨点数ぶんに絞る。**見出しの点数とチップ数は必ず一致する。**
- 推奨に選ぶ順序は展開順（軸1の前進 → 軸1の逆方向 → 軸2の前進 …）。

##### 🔴 点数は 3 種類あるので混同しない

| # | 名前 | 値の例（12頭・通常） | 使い道 |
|---|---|---|---|
| 1 | **推奨購入点数** | **10点 / ¥1,000** | 画面に出す |
| 2 | 展開した組の総数 | 16点 | 内部（`allCombos` / `expandedPoints`）。画面に出さない |
| 3 | 回収率の投資基準 | **全レース5点固定** | `archiveResults` の回収率（BET_POINT_LOGIC.md） |

🔴 **1 は 2 の部分集合である。** したがって
**推奨から外れた組で決着した場合、公開実績（2・3 に基づく）が的中でも、
推奨どおり買った人は外れる。** この差は仕様所有者の判断で受け入れている。
...
| レース | 頭数 | 買い目 | 展開 | **推奨（画面表示）** |
|---|---|---|---|---|
| 通常 1R | 12 | `4-6.8.12.3.11.9(抑え10.7.5)` ＋ `6-…` | 16点 | **10点 / ¥1,000** |
| メイン 11R | 9 | `6-4.1.3.7.8` | 5点 | **5点 / ¥500** |
