...
| R-3 | 無料会員の印は **「印」1 列に複数を重複付与**する。◎3〜5 / ○3〜5 / ▲3〜5 / △約10 頭を重ね、該当しない馬は空欄。**本命は分かってよい**（評価最上位だけが ◎○▲△ の 4 つすべてを持つ＝印が最も多い馬）。守るのは**相手が誰か**であり、△ を買い目の相手（5〜6 頭）より広く取ることで担保する | free |
...
「印」は **1 列だけ**。評価項目ごとの列は作らない。**1 頭に複数の印**を重ねて付ける
（新聞で複数の記者印が横に並ぶのと同じ密度をねらう）。
...
🔴 **評価最上位だけが 4 つすべての印を持つ。** 印が多い馬ほど強い、という
直感どおりに読めるようにするため。

> **失敗の記録（2026-08-29）**: 当初は最上位を「◎△」にして一意にしていた。
> 一意ではあったが、2〜4 位の「◎○△」より印が **少ない**ため、
> 画面上で最上位が最も弱く見え、**本命として読めなかった**。
> 「印の数 ＝ 強さ」という読み手の直感を無視した設計ミスだった。
...
- **本命は分かってよい。** 評価最上位だけが ◎○▲△ の 4 つすべてを持つため、
  **印が最も多い馬＝本命**として一目で読める。
- **相手は絞り込ませない。** △ を **買い目の相手より広く**（約 10 頭）取る。
  ◎○▲ を明示しても、正解の 5〜6 頭は △ プールに埋もれて特定できない。
...
- 印の算出 … `src/utils/attentionMarks.js`
  （`evaluationOrder` → `computeMarkBands` → `assignFreeMarks`。Math.random / Date に依存しない）
