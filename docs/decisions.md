...
## 2026-09-11 継続リワードの対象決済方式・移行時の引継ぎ・買い切り会員の扱い

**決定**（仕様所有者）

1. 継続リワード（ランク / 継続月数 / KIリワード / 継続特典・交換品）の**主対象は
   クレジットカードによる継続決済**。これらは**クレジット継続決済の価値**として位置づける。
2. **銀行振込**の月払い・年払い会員は**現在の契約・閲覧権限を変更しない**。
   クレジット継続決済へ**移行すると**継続リワードを受けられる導線とする。**強制移行はしない**。
3. **銀行振込 → クレジットへ移行した場合、それまでの正当に確認できる継続月数を引き継ぐ**。
   引き継いだ月数に応じて **Bronze / Silver / Gold / Platinum のランクも継続する**。
4. **買い切り・永久会員**は継続ポイント蓄積の主対象にしない。
   代わりに**専用の称号・バッジ**を与える方向。**既存の永久閲覧権限・契約条件は変更しない**。

**理由**

継続リワードは「継続して支払い続けていること」への対価であり、
継続決済という支払い形態そのものと結びつく。一方で、既存の銀行振込会員・買い切り会員から
**現在の契約や閲覧権限を取り上げない**ことを同時に確定した。

**根拠に使ってよいデータ**

- `RewardLedger` の支払い済み期間 / `MembershipStartedAt`（＝支払い成功日）
- 🔴 **`CreatedAt`（申込日）は加入日として使わない**。支払っていない期間を数えないため。
- 🔴 根拠不明の期間は**推測しない**。`pending`（準備中）にする。

**実装への影響**

- `resolveTenureMonths()` に**台帳が始まる前の期間**の引継ぎを追加した。
  これが無いと、移行した瞬間に継続月数が台帳の 1 件ぶんへ落ちて**降格**していた。
- `rewardScope.js`（新規・純関数）で支払い方法と対象範囲を判定する。
- 買い切りへの付与は `airtableStore.appendEntry()`（**全付与経路の唯一の出口**）で止める。

**確定していないこと**

- TBD-14: 銀行振込の期間を今後も台帳へ積み続けるか（現在は積んでいる。**止める変更は行わない**）
- TBD-15: 買い切り会員の**称号名・表示文言**（候補のみ。表示に出さない）
- TBD-16: 銀行振込期間の**過去ポイントの遡及付与**（🔴 承認なしに実装しない）
