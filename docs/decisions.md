...
本リポジトリには `README.md` / `DESIGN.md` / `CLAUDE.md` / `BET_POINT_LOGIC.md` / `docs/` 配下 10 本など
多数の文書が存在するが、**現在のスコープ・進捗・未確定事項を一箇所で示す文書がなかった**。
...
2. **`CLAUDE.md` だけを更新し新ファイルを作らない** — `CLAUDE.md` は既に 667 行あり、運用ルール・仕様・進捗・環境変数が混在している。役割分離が進捗追跡の前提であるため不採用。
...
- 南関 110 開催 1378R を含む archive JSON が大きく書き換わった（差分 7,729 挿入 / 6,069 削除）。
...
- 一方で workflow 数は Phase 1 当時の 8 本から **現在 13 本に増加**しており、Phase 2 の統合計画（8→5）は前提が変わっている。
- `pull --rebase` リトライループは各 workflow に残存している。

### Revisit Conditions

- 競合が完全に消えたことを確認できた時点で Phase 3（rebase ロジック削除）を判断する。
- Phase 2（統合）は現在の 13 workflow を前提に再計画が必要。
