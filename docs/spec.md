...
- 継続リワードの**主対象は、クレジットカードによる継続決済の会員**（同 §7.10）。
  🔴 **銀行振込・買い切りの会員から、現在の契約・閲覧権限を取り上げない。**
  銀行振込からクレジットへ移行した会員は、**正当に確認できる継続月数とランクを引き継ぐ**。
  🔴 根拠は repo 内の正本データ（`RewardLedger` / `MembershipStartedAt`）だけで、
  **`CreatedAt`（申込日）を加入日として使わない**。根拠不明の期間は**推測しない**。
