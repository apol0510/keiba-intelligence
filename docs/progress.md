...
- **発送管理画面は作らない。** Airtable の `RewardRedemptions` を当面の発送キューとし、
  運用者が **`approved`** を見て発送し `Status` を `shipped`・`ShippedAt` を更新する
...
#### 🔴 書く順序（3 段階。2026-09-07 に片側成功を修正）

**① `requested` を積む → ② 台帳の減算を冪等に成立させる → ③ 成立したときだけ `approved`。**

🔴 **当初の実装（`5ed5da74`）には回復不能な片側成功があった**（仕様所有者の指摘）。
キューへ `requested` を作れたあと減算が失敗すると**交換行だけが残り**、
再送は既存 `RedemptionId` を見つけた時点で無条件に `already` を返していたため、
**減算が永久に再試行されなかった**。しかも当時の docs は「`requested` を見て発送」と
書いていたため、**ポイントを引かずに発送する**運用になりえた。

修正:

| 変更 | 内容 |
|---|---|
| `requested` の意味 | **申込予約**。🔴 **発送対象ではない** |
| `approved` の意味 | **減算まで成立した**＝ここで初めて発送してよい（記念品は 0 pt で成立扱い）|
| 再送時 | `approved` / `shipped` / `cancelled` → `already`。**`requested` → 減算の有無を確認して引き直し、成立したら `approved`** |
| 二重減算 | 台帳に同じ `entryId` があるかを見てから引く（**G-30**）|
| 運用 | 🔴 発送対象を **`approved` のみ**へ docs も含めて統一 |

| 失敗箇所 | 残る状態 | 会員のポイント | 回復 |
|---|---|---|---|
| ① キューへ積めない | 何も残らない | 減らない | 再送でやり直せる |
| ② 減算が失敗 | **`requested`**（発送されない）| 減らない | **再送で引き直し → `approved`** |
| ③ `approved` にできない | `requested`（発送されない）| 引かれている | 再送で `approved`（減算は冪等）|

静的ガード: **G-27**（キュー → 減算 → approved の順）/ **G-29**（`requested` を terminal にしない）/
**G-30**（既存エントリを見てから引く）。
...
- 🔴 再送の短絡は**残高を見る前**に置いた。あとに置くと、1 回目で引かれた結果
  2 回目が `insufficient_points` になり、**成功した申込が失敗として返る**（実装中に発見・修正）
- 🔴 ただし短絡は**状態を見てから**行う。無条件に `already` を返すと
  減算が失敗した申込が回復しない（上記の修正）
...
| snapshot | 住所を変えても過去行が変わらない / 減算まで通れば `approved` になる |
| **片側成功からの回復** | キュー成功 → 減算失敗 → 再送で**減算が 1 回だけ成立し `approved`** / 減算失敗中は**発送対象にならない** / 減算済みの再送で**二重に引かない** / `approved`・`shipped` の再送は**何も書かない** / 回復時に残高不足なら `approved` にしない / 記念品は 0 pt のまま `approved` |

#### 検証（ローカル実測。GitHub checks は根拠にしない）

| 検査 | M9 時点 | M11 後 |
|---|---|---|
| `test:membership` | 210 | ✅ **251**（fail 0）|
...
### Membership Phase の残件（2026-09-08 整理）— 仕入れの実行 / TBD-13

制度・実装・スキーマ・本番反映は完了した。**残っているのは運用側の 2 件だけ**で、
どちらも **仕様所有者の判断**が要る。🔴 Claude は数値を決めない。

正本: `MEMBERSHIP_REWARDS.md` §7.8（品目）/ §7.5（未確定一覧）

#### 残件 A — 景品の仕入れの実行（TBD-3b / TBD-4b の実行側）

**品目は確定済み**（米 / コーヒーの2択）。残るのは**実際に仕入れて送れる状態にすること**。

| 確定していること | 値 |
|---|---|
| 仕入れ単位 | 米 **15kg ¥11,000** / コーヒー豆 **1kg ¥3,000** |
| 払い出し量（600 pt・12 か月）| 米 約300g / コーヒー豆 50g |
| 払い出し量（1,200 pt・24 か月）| 米 約450g / コーヒー豆 100g |
| 原材料原価 | 米300g≈¥220 / 450g≈¥330 / コーヒー50g=¥150 / 100g=¥300 |
| 加工 | まとめ仕入れ → **KI 側で小分け・ラッピング** |

🔴 **未定**: 仕入れ先・発注のタイミング・在庫の持ち方・小分けとラッピングの作業者と資材。

##### いつまでに必要か（実測にもとづく見積り）

**当面は発送が発生しない見込み。** 交換の最低ラインは **600 pt** で、
残高は **`RewardLedger` の支払い成功エントリからのみ**数える（`summarizeRewards`）。
付与は 100 pt / 月なので、**600 pt には支払い成功 6 期ぶんが要る**。

2026-09-07 時点の `RewardLedger` は **1 行**（read-only 実測）。
したがって現時点で 600 pt に到達できる会員は居ない。

🔴 ただしこれは**時点の観測**であり、年払い会員（1 期 = 1,200 pt 一括）が入れば
**その 1 件で 1,200 pt に到達しうる**。**「6 か月は猶予がある」と決めつけないこと。**

##### 監視すべきこと

交換の受け皿は**本番で稼働している**（カタログ `published`・交換 API は 401/200 を返す状態）。
在庫が無いまま申込が入ると、**`RewardRedemptions` に `approved` が積み上がって発送できない**。

- 見る場所: Airtable `RewardRedemptions` の **`Status = approved`**
- 🔴 `requested` は**発送対象ではない**（ポイント減算が未成立の回復待ち。§7.9）
- 確認コマンド: `npm run membership:check`（`RewardRedemptions` の状態内訳を表示する）

#### 残件 B — TBD-13（包装資材費と景品価額 `valueYen`）

| 項目 | 状態 |
|---|---|
| 原材料原価 | ✅ 判明（上表）|
| **包装資材費** | 🔴 **未確定** |
| `valueYen` | 🔴 **未設定**（`rewardCatalog.json` の全 8 品）|

##### なぜ未設定のままにしてあるか

原材料原価だけを景品価額として登録すると**実態と食い違う**ため。
🔴 **包装費込みの推測値・架空値を入れない**（`MEMBERSHIP_REWARDS.md` §7.8）。

現状の挙動: `catalog.js` は `valueYen` が **`null` なら ¥796 の検査を素通しする**
（`if (item.valueYen != null)` の中でしか検査しない）。
つまり **保守ライン S-1 の自動検査が効いていない**状態である。
🔴 **`MAX_ITEM_VALUE_YEN = 796` 自体は変更していない。**

##### 確定したら何をするか（手順）

1. `rewardCatalog.json` の各 item に `valueYen` を入れる（**¥796 以内**）
2. 🔴 **静的ガード G-23 を同時に更新する。**
   現在は `assert.equal('valueYen' in item, false)` で**不在を固定**しているため、
   値を入れると **テストが落ちる**（`membershipCopy.guard.test.mjs`）。
   これは意図した設計で、「推測値をこっそり入れる」ことを防いでいる。
3. `MEMBERSHIP_REWARDS.md` §7.5 の TBD-13 と §7.8 の記述を確定内容へ

##### 🔴 決めてはいけないこと

**景品類の価額に何を含めるか（原材料費のみか、包装・送料を含むか）は
景品表示法の解釈であり、コード内・docs で断定しない**（§9-7 / §8.2）。
必要なら専門家の確認を経ること。Claude は**言われた値を入れるだけ**にする。

#### まとめ

| # | 残件 | 誰が | いつ |
|---|---|---|---|
| A | 景品の仕入れの実行 | **仕様所有者** | 最初の `approved` が出る前 |
| B | TBD-13（包装資材費 → `valueYen`）| **仕様所有者**（必要なら専門家確認）| S-1 の自動検査を効かせたいとき |
...
| `AIRTABLE_BASE_ID` | ✅ **登録済み**（2026-09-08。当初は未登録で、仕様所有者が追加）|
| `ALERT_EMAIL` / `SENDGRID_API_KEY` / `SENDGRID_FROM_EMAIL` | ✅ 既存 |

🔴 secret 追加は承認境界のため Claude は行っていない（仕様所有者が実施）。
...
### 2026-09-08 発送待ち監視を本番で初回実行 — ✅ 正常終了・通知なし・write なし

PR #114 を merge（**`52bc3b90`**）し、`workflow_dispatch` で**手動実行して動作を確認**した。

#### 実行結果

| 項目 | 値 |
|---|---|
| run | [34194999690](https://github.com/apol0510/keiba-intelligence/actions/runs/34194999690)・**success** |
| 監視ステップ | ✅ success — `監視成功: approved 0 件 / requested 0 件` |
| **通知ステップ** | ✅ **skipped**（`approved_count != '0'` が false）＝ **メールを送っていない** |
| 所要 | 約 20 秒 |

**正常時（0 件）にメールを送らない**ことを実機で確認できた。

#### Airtable への影響（実行前後を read-only 比較）

| 対象 | 実行前 | 実行後 |
|---|---|---|
| `RewardRedemptions` | 0 行 | **0 行**（不変）|
| `RewardLedger` | 1 行 | **1 行**（不変）|
| 列・`Status` の選択肢 | 揃っている | **不変** |

🔴 **write は 1 件も発生していない**（監視は GET のみ）。

#### ログの安全性

| 検査 | 結果 |
|---|---|
| PAT / base id がログに出ていないか | ✅ なし（GitHub が **5 箇所を `***` でマスク**）|
| メールアドレス | ✅ なし |
| 受取人氏名・郵便番号・住所 | ✅ なし |

#### 監視の現在地

- **日次 9:00 JST** で自動実行される（次回から）
- `approved` が 1 件以上になったときだけ `ALERT_EMAIL` へ通知
- 取得失敗・認証失敗・スキーマ不一致は **failure(red)**。「0 件」として静かに緑にならない
- 🔴 `requested` は発送対象ではない（減算未成立の回復待ち）

これで**残件 A（景品の仕入れ）の見落としに気づける状態**になった。

### 2026-09-08 プレゼントカタログを商品カードで表示（`/mypage` のみ・データ駆動）

確定済みの景品を、会員が**事前に見られる商品カタログ**として `/mypage` に表示した。
正本: `MEMBERSHIP_REWARDS.md` §7.8（品目）/ §4（品目を出してよい場所）

#### 表示する場所

🔴 **`/mypage` の景品エリアだけ。** `/pricing` と `/terms` には品目を出さない。
静的ガード **G-31** が、`rewardCatalog.json` の**実際の品目名**を読み込んで
`/pricing` `/terms` に含まれていないことを検査する（語句のハードコードではなくデータ駆動）。

#### 実装

| 層 | 変更 |
|---|---|
| `catalog.js` | `image` の受け渡しを追加（**任意**）。実写真が用意できたら **JSON にパスを足すだけ**で差し替わる |
| `membershipView.js` | `catalog` ビューを新設（**表示専用**）。ライン別に `affordable` / `remainingPoints`、記念品別に `reached` / `monthsToGo` |
| `mypage.astro` | テキスト一覧 → **商品カード**。ライン見出し ＋ 「どれか 1 つ」の明示 ＋ カード grid |

🔴 **交換ロジック・entitlement・`RewardRedemptions`・ポイント減算条件は一切変更していない。**
`catalog` ビューは `exchangeView` の結果を書き換えず、**申込の可否は従来どおり `gifts` 側**が決める。

#### 表示の決まり

| 状態 | 見せ方 |
|---|---|
| 交換できる（`affordable`）| バッジ「交換できます」・カードは通常表示。**既存の申込フォームへ接続** |
| ポイント不足 | **カタログには出す**が `is-locked` で淡くし、バッジは「あと ◯ pt」。申し込めるとは見せない |
| 残高が不明 | `affordable=false` ＋ 「準備中」。🔴 **「あと◯pt」を推測で出さない** |
| 記念品（12 / 24 か月）| 同じカタログデータから対象商品を表示。到達で「対象です」、未到達は「あと ◯ か月」 |
| 記念品の月 | 通常交換は S-2 どおり止まる（従来の注意書きを維持）|

🔴 **`valueYen`・円換算・景品価格は表示しない。**
🔴 **`requested` / `approved` 等の運用状態はカタログへ混ぜない**（ガード G-33）。

#### 商品画像

repo を調べたところ、**実商品の写真は存在しない**（サイト用のブランド画像のみ）。
🔴 **架空の写真・別商品の写真は当てていない。** 画像未登録でもカードとして成立するよう、
プレースホルダ（🎁）で高さを確保してある。
将来 `rewardCatalog.json` の各 item に `image` を足せば、**コード変更なしで差し替わる**。

#### レスポンシブ（実測）

`buildMembershipView` の実出力と `mypage.astro` の実 CSS から静的プレビューを作り、
ブラウザで確認した（検証用。repo へはコミットしていない）。

| 幅 | 結果 |
|---|---|
| PC（1280px）| ✅ 1 ライン 2 カード。交換可能ラインは通常表示、不足ラインは淡色 ＋「あと 500 pt」 |
| mobile（≤480px の media query 適用時）| ✅ **1 カラム**へ落ち、品名が折り返さず読める |

🟡 macOS のウィンドウ最小幅の制約で **viewport を 480px 以下にできなかった**ため、
mobile は「media query の中身が正しいこと」を breakpoint を一時的に広げて確認した。
breakpoint の値（480px）そのものは実機で未確認。

🟡 `npm run dev` はこのリポジトリでは**元から起動しない**
（`netlify.toml` の base が `astro-site` から解決されず `astro-site/astro-site` を探す）。
本変更とは無関係の既存事象で、**修正していない**。

#### テスト（+15 → membership **266**）

| 追加 | 内容 |
|---|---|
| 表示ビュー 11 件 | published から描画 / **draft は 1 品も出さない** / 600・1,200 とも **2 択を 1 商品へ絞らない** / ポイント不足は出すが `affordable=false` / **残高不明なら「あと◯pt」を出さない** / 記念品の到達状況 / 運用状態を混ぜない / 価額を出さない / 画像は未登録なら null・データから渡る |
| ガード 4 件 | **G-31** 実品目名が `/pricing` `/terms` に漏れない / **G-32** `/mypage` も直書きせずカタログから描画 / **G-33** 運用状態・価額を混ぜない / **G-34** ポイント不足を交換可能に見せない |

既存の **G-25**（品目名をコード・UI へ直書きしない）は維持。

#### 検証

`test:membership` **266** / `test:auth` **150** / `test:ai-auth` **11** / `test:billing` **61** /
`test:stripe` **52・17・6** / `test:narrative` **107** / `test:reward-alert` **21** /
`npm run build` **exit 0**（15 スイート fail 0）。すべて fail 0。
...
### 2026-09-08 カタログの本番修正 2 件（社内情報の露出 / 開閉）— PR #118・#119

本番で仕様所有者が実物を確認し、**3 件の問題**が見つかったので順に直した。

| # | 指摘 | PR | merge commit |
|---|---|---|---|
| 1 | **カードに見えない**（枠線が出ていない）| [#118](https://github.com/apol0510/keiba-intelligence/pull/118) | `6a41807c` |
| 2 | **社内情報が顧客に出ている** | [#119](https://github.com/apol0510/keiba-intelligence/pull/119) | **`f75be140`** |
| 3 | **カタログが畳めない** | 同上 | 同上 |

#### 🔴 1. カードに見えなかった（#118 / `6a41807c`）

`var(--border)` を使っていたが、**この変数はサイトに存在しない**
（正しくは `--border-color: #dbe2ea` / `global.scss:164`）。
CSS は未定義変数を参照すると **宣言ごと無効**になるため、**border が黙って消えていた**。エラーは出ない。

🔴 **merge 前の目視確認をすり抜けた理由**: 自作の検証ハーネスで `--border` を
**自分で定義してしまっていた**。実サイトのトークンを使わずに「見た目 OK」と判断したのが誤り。
以後ハーネスは `global.scss` の `:root` をそのまま読む方式にした。

- `var(--border)` → `var(--border-color)` を **6 箇所**
  （カード / サムネイル / バッジ / ライン区切り / **申込フォーム 2 箇所**）
  🔴 **申込フォームの枠線も M11 から消えていた**ので併せて直った
- カードの面を既存トークンで付与（`--bg-muted` / `--radius-md` / `--radius-pill`）
- **G-35** を追加。UI が使う `var(--x)` が `global.scss` / `BaseLayout.astro` に
  定義されているかを静的に検査する（fallback 付きは対象外）。
  わざと `var(--border)` へ戻すと **G-35 が落ちる**ことを実測で確認した。

#### 🔴 2. 商品説明に社内情報を出していた（#119 / `f75be140`）

商品説明に **「まとめ仕入れした米を KI 側で小分け・ラッピングしてお送りします。」**
と書いており、**調達方法と社内作業を会員に見せていた**。M9 でカタログを作った際に入れた文言。

- `rewardCatalog.json` の **交換品 4 件 ＋ 記念品 4 件、計 8 件の `description` を削除**
- 🔴 **勝手な宣伝文句へ置き換えていない**（商品の主張を作らない）。
  品名（`米 約300g（2合）` 等）だけで必要な情報は足りる
- 記念品の説明は見出し・バッジと重複していたため、削除でカードも読みやすくなった
- `note` に方針を明記: **顧客向け説明を入れる場合も社内の運用・原価・在庫に触れない**

**G-36** を追加。`description` に
**仕入れ / 小分け / ラッピング / 原価 / 在庫 / 発注 / 調達**
が入っていないことを静的に検査する。

#### 3. カタログを開閉できるようにした（#119 / `f75be140`）

縦に長く畳めなかったため、`<details>` / `<summary>` で開閉できるようにした（**JS 不要**）。

- 🔴 **交換できる品がある場合だけ初期 `open`**（`hasAffordableGift`）
- 🔴 **閉じていても `交換できます` バッジが `summary` に出る**ので見落とさない
- 三角マークで開閉の向きが分かる

**G-37** を追加。`details` / `summary` で畳める構造を固定する。

ガードの追随:

- **G-33** の探索起点を `<h3>` から `<details class="mp-cat">` へ
- **G-34 を強化** — 「交換できます」を出す箇所を**全数走査**し、すべてが
  `group.affordable` または `hasAffordableGift()` で守られていることを検査。
  `hasAffordableGift` が `g.affordable` から作られていることも固定

#### 変更していないもの

🔴 **表示ロジック・entitlement・交換条件・ポイント減算は一切変更していない。**
#118 は CSS のみ、#119 はカタログのデータ（`description`）と表示の開閉のみ。

#### 検証

| 検査 | 結果 |
|---|---|
| `test:membership` | ✅ **269 PASS**（fail 0）|
| `npm run build` | ✅ **exit 0**（15 スイート fail 0）|
| production published | ✅ **`f75be140` / ready**（`error_message` なし）|
| 見た目 | ✅ 実 `global.scss` のトークンで**開いた状態・閉じた状態の両方**を目視確認 |

本番 read-only 確認（🔴 **write は一切していない**）:

| 対象 | 結果 |
|---|---|
| `/` `/pricing` `/mypage` | ✅ **200** |
| guest → `/prediction/nankan` | ✅ **302**（fail-closed 維持）|
| `POST /.netlify/functions/redeem-reward`（未認証）| ✅ **401 `login_required`**（fail-closed 維持）|
| guest の `/mypage` | ✅ 品目名・**仕入れ / 小分け / ラッピング**・カタログいずれも**出ていない** |
| **Airtable** | ✅ **write なし**（read-only の確認のみ）|
...
## 2026-09-12 UAT 決済前 baseline の記録と、無料会員表示の意味的不整合の修正

### 1. 決済前 baseline（仕様所有者が UAT `/mypage` で観測）

修正前の UAT `/mypage`（**無料会員としてログインした状態**）:
...
| 会員ランク | **準備中** |
| 継続月数 | **準備中** |
| `RewardLedger` 由来のポイント | **0 pt** |

🔴 これは **Test Mode 決済前の baseline** として記録する。
決済後に 7 項目（契約価格 / 価格ロック / ランク / 継続月数 / 残高 / 今月の積み上げ /
次ランク進捗）が**実値**へ変わることを確認する。
🔴 **決済後も「準備中」が残る場合は完成扱いにせず、その場で原因を切り分ける**
（webhook → Airtable → `refresh-session` の順に追跡する）。
🔴 UI 修正による見かけの辻褄合わせ・仮値の投入はしない。

### 2. 無料会員表示の意味的不整合を修正

baseline の観測から、**表示の意味づけ自体が誤っている**ことが判明した。

- 無料会員は「こちらがデータを出せていない（＝準備中）」のではなく、
  **プレミアムを始めていないので制度が動いていない（＝未開始）**
- 「準備中」を出すと、**正常な状態がこちらの不具合のように見える**
- 同様に「あと◯pt」「次のプレゼントまでの進捗」も、制度が始まる前に出す意味がない

正本 `docs/MEMBERSHIP_REWARDS.md` に沿って、表示状態を次のとおり分けた。

| 状態 | 契約価格 / 価格ロック | ランク・継続月数・リワード・次ランク | 次のプレゼント進捗 |
|---|---|---|---|
| **無料会員（未開始）** | **—** | **プレミアム加入後に開始** | **出さない** |
| プレミアム（データ未確定）| 準備中 | **準備中**（従来どおり）| 準備中（従来どおり）|
| プレミアム（データあり）| 実値 | 実値 | 実値 |

#### 🔴 解約済み会員を巻き込まない

解約すると tier は無料へ戻るが、**ポイントは 90 日間は生きている**（§7.1）。
ここを一律「未開始」にすると、残高と猶予の案内が消えてしまう。
そこで未開始の判定は **`!isPaid` かつ会員履歴が 1 つも無いとき**に限定した
（`membershipStartedAtIso` / `contractPrice` / `cancelledAtIso` / 台帳エントリのいずれも無い）。

#### 変更点

| ファイル | 内容 |
|---|---|
| `src/lib/membership/membershipView.js` | `MEMBERSHIP_STATUS.NOT_STARTED` と `notStarted` を追加。未開始なら status を `not_started` にし、**数値・進捗を null** にする |
| `src/pages/mypage.astro` | 未開始は「—」「プレミアム加入後に開始」を出し、**進捗ブロックとカタログのバッジを描かない** |
| `membership.test.mjs` | 回帰テスト 7 件（未開始 / 進捗非表示 / guest / プレミアムは pending 維持 / 解約済み・台帳ありを未開始にしない / 文言が別物）|
| `membershipCopy.guard.test.mjs` | 静的ガード 5 件（`PENDING` 直書きの禁止・進捗ブロックの位置・バッジ抑止・契約価格の `—`）|

🔴 **entitlement・認可・ポイント計算ロジックは変更していない。**
`rewards.js` / `ranks.js` / `priceLock.js` / `tiers.js` に差分は無く、変更は**表示の分類だけ**。

#### テスト

`test:membership` **356 pass** / `test:auth` **180 pass** / `test:billing` **61 pass** / `astro build` **成功**。
変異テストで有効性を確認:
`CLUB_ROWS` を `PENDING` 直書きへ戻すと **guard 1 件 fail**、
`notStarted` を常に false にすると **2 件 fail**。いずれも戻すと全件 pass。
...
## 2026-09-12 プレミアム専用グラデーション `--grad-premium` を新設（青→紫→橙）

桃→橙（`--grad-action`）は**色差が弱くプレミアム感が出ない**という判断で、
ロゴのブランドカラーに寄せた**プレミアム専用トークン**を新設した。

### ブランドの裏取り

ロゴ `.logo-text` は `linear-gradient(135deg, var(--primary-start), var(--secondary-start))`
＝ **#2563eb → #f97316**。その青と橙のあいだに紫を挟み、並びを保ったまま 3 色にした。

### 🔴 端点を落とした理由（コントラスト実測）

当初案の `#2563eb → #8b5cf6 → #f97316` は、**どの文字色でも AA(4.5:1) を満たせない**。
明暗が逆方向へ振れるため、単一の文字色では挟み撃ちになる。
ランプ全体を 200 分割した**最小**コントラスト（停止点だけでなく補間途中も含む）:

| 案 | 停止点 | 濃色 #0b1020 | 白 #ffffff | 判定 |
|---|---|---|---|---|
| 旧 `--grad-action` | #ec4899 → #f97316 | 5.37 ✅ | 2.80 ❌ | 濃色で成立 |
| 当初案 | #2563eb → #8b5cf6 → #f97316 | 3.66 ❌ | 2.80 ❌ | 🔴 **不成立** |
| 別案（紫→桃→黄） | #7c3aed → #ec4899 → #facc15 | 3.32 ❌ | 1.53 ❌ | 🔴 **不成立** |
| **採用** | **#2563eb → #7c3aed → #c2410c** | 3.08 ❌ | **5.17 ✅** | **白文字で成立** |

色相は変えず**明度だけ**下げている。青の始点はロゴと同一（`--primary-start`）。
バッジは 0.72rem/800（約 11.5px）で WCAG の「大きな文字」に当たらず、3:1 への緩和は使えない。

### 適用先（この 3 箇所だけ）

| 箇所 | セレクタ |
|---|---|
| プレミアムカード枠 | `.mp-status.is-paid` |
| プレミアムバッジ | `.mp-badge.tier-premium`（文字は `--text-inverse`＝白） |
| 進捗バー | `.mp-prog-bar span` |

🔴 押せる UI（`--grad-*-btn`）には使わない。🔴 無料会員・ライトには使わない。
🔴 既存 gradient トークン（`--grad-action` / `--grad-conclusion` / `--grad-nav`）は不変。

### 回帰テスト

`src/lib/membership/premiumGradient.guard.test.mjs`（**19 件**）を新設し、`test:membership` に組み込んだ
（`npm run build` にも載る）。固定した内容:

- `--grad-premium` の値と停止点、青の始点が `--primary-start` と一致すること
- **3 箇所が同じトークンを参照**していること／旧トークンが残っていないこと
- 白文字で AA 成立・濃色文字では不成立（＝白が必須である理由）を実測で固定
- 無料会員・ライトのバッジ、既存 gradient トークン、ボタン用トークンが不変であること
- `--grad-premium` の使用が mypage の 3 箇所**だけ**であること

🔴 この配色は 3 回作り直しており、そのたびに 3 箇所のどれかが取り残された
（バッジと進捗バーだけ直してカード枠が単色オレンジで残った実績がある）。
機械で 3 箇所の一致を固定したのはそのため。

変異テストで有効性を確認: 進捗バーだけ旧トークンへ戻すと **4 件 fail**、
バッジ文字を濃色へ戻すと **1 件 fail**、トークンを当初案（AA 不成立の色）にすると **1 件 fail**。
いずれも戻すと 19/19 pass。

### 確認

`test:membership` **375 pass** / `test:auth` 180 / `astro build` 成功。
**ビルド成果物の実 CSS を読み込んだ**ハーネス（PC 760px / モバイル 390px）で、
3 箇所の `background-image` が**文字列として完全一致**し、カード枠が角丸（20px）に追従し、
無料会員カードが従来どおり（`#dbe2ea` 2px・グラデーション無し）であることを確認した。

## 2026-09-12 `--grad-premium` を明るい 3 色（空→藤→桃）へ差し替え

前項の暗色 3 色（`#2563eb → #7c3aed → #c2410c`）は、UAT の実画面で
**終点が茶色く濁って見え**、プレミアム感を損ねていた。仕様所有者の判断で
**明るい 3 色 ＋ 濃色文字**へ切り替えた。

```
--grad-premium: linear-gradient(135deg, #38bdf8 0%, #a78bfa 50%, #f472b6 100%);
```

### 選定の経緯

4 案（青→藤→橙 / 空→藤→桃 / 青→藤→琥珀 / 藤→桃→橙）を、UAT 配信中の実 CSS を
土台にした PC / モバイル実寸ハーネスで比較し、仕様所有者が **空→藤→桃** を選択した。

### 🔴 「濁り」を数値で扱えるようにした

茶色く見える原因は 2 つあった。**終点 #c2410c 自体が暗い錆色**であることと、
**藤から暖色へ補間すると途中で彩度が大きく落ちる**こと。
そこでランプ全体の**最小彩度**（HSL の S）を濁りの指標として測った。

| 案 | 停止点 | 濃色文字 | 白文字 | 最小彩度 |
|---|---|---|---|---|
| 旧（暗色3色） | #2563eb → #7c3aed → #c2410c | 3.08 ❌ | 5.17 ✅ | 43% |
| 青→藤→橙 | #60a5fa → #a78bfa → #f97316 | 6.37 ✅ | 2.54 ❌ | 30% |
| **採用** 空→藤→桃 | **#38bdf8 → #a78bfa → #f472b6** | **6.85 ✅** | 2.14 ❌ | **50%** |
| 青→藤→琥珀 | #60a5fa → #a78bfa → #fbbf24 | 6.96 ✅ | 1.67 ❌ | 22% |
| 藤→桃→橙 | #a78bfa → #f472b6 → #fb923c | 6.85 ✅ | 2.26 ❌ | 50% |

桃で終わる並びは最後まで彩度が落ちない。🔴 **終点に橙・琥珀を置かないこと。**

### 🔴 新しい色を足していない

3 色ともすでに設計システムにある色である。
`#38bdf8 → #a78bfa` は **`--grad-nav-bright` そのもの**（＝フッターの無料会員登録ボタン）、
`#f472b6` は `--grad-conclusion-btn` / `--grad-action-btn` の桃。

### 文字色

バッジ文字は **`--btn-ink`（濃色）へ戻した**。実測（ランプ全体 400 分割の最小値）で
濃色 **6.85 → AA ✅** / 白 2.14 → 不足。これで押せる UI の正本
（「明るい 2 色 ＋ 濃色文字」）とも揃った。

### 回帰テスト

`premiumGradient.guard.test.mjs` を **21 → 24 件**へ拡張。

🔴 **コントラストと彩度を `global.scss` の実値から測るよう直した。**
以前は定数側を測っていたため、値を差し替えても「確定値が違う」としか落ちず、
その配色が実際に AA を割るのか濁るのかをテストが検査できていなかった。

追加で固定した内容:

- **差し戻し済みの 4 配色へ戻っていない**こと（値と理由を列挙して保持）
- ランプ全体の**最小彩度が 40% 以上**であること（濁りの下限）
- 3 色とも既存トークンに登場する色であること／空→藤が `--grad-nav-bright` と一致すること
- バッジ文字が `--btn-ink` で、白文字（`--text-inverse`）はバッジ基底の 1 箇所だけであること

変異テスト（いずれも戻すと 24/24 pass）:
暗色 3 色へ戻すと **2 件 fail** ／ 琥珀終点（彩度 22%）にすると **2 件 fail**（うち 1 件は彩度テスト）／
濃色文字が AA を割る暗い配色にすると **2 件 fail**（うち 1 件はコントラストテスト）／
進捗バーだけ旧トークンへ戻すと **4 件 fail**。

### 確認

`test:membership` **380 pass** / `test:auth` 180 / `astro build` 成功。
ビルド成果物の実 CSS を読み込んだハーネス（PC 760px / モバイル 390px）で、
3 箇所の `background-image` が完全一致（`rgb(56,189,248) → rgb(167,139,250) → rgb(244,114,182)`）、
バッジ文字 `rgb(11,16,32)`、カード枠が角丸 20px に追従、
無料会員カードとバッジ（`--success`）が不変であることを確認した。
