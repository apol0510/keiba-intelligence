...
### 2026-09-02 ログイン後の遷移先: 未承認の仕様変更を取り消し

- **経緯**: 「無料会員のログイン後はマイページが正しい」との指摘を受け、
  `verify-magic-link` の `redirectTo` を全 tier `/mypage` へ変更した（`1f509647`）。
- **正本の確認結果（read-only）**:
  - `docs/spec.md` / `docs/decisions.md` / `docs/RENEWAL_2026_08.md` / `CLAUDE.md` の
    いずれにも **ログイン後の遷移先の規定は無い**（`redirectTo` の語も出てこない）。
  - 現行の分岐は **コードにのみ存在**する。`3cdd0c4e`（2026-08-30・仕様所有者の指示）で
    会場別分岐（`venueAccess` による `/prediction/jra` `/prediction/nankan`）を廃止した際に、
    `無料 → /free-prediction` ／ `有料 → /mypage` の形になった。
    ただしコミット本文にも遷移先の変更意図は書かれていない。
  - よって「全 tier `/mypage`」は正本に無く、**未承認の仕様変更**である。
- **対応**: `1f509647` を revert し、従来の遷移仕様へ戻した。
  - `無料 → /free-prediction` ／ `有料（light / premium） → /mypage`
  - 購入導線のみ `resumePathFor` が固定パスで上書き（不変）
- **文言は実際の遷移先に合わせたまま維持**:
  購入導線「メールアドレスを確認しました／このままお支払い画面へ進みます…」、
  通常ログイン「ログインしました／（`/mypage` ならマイページへ、
  `/free-prediction` なら今日の予想へ）移動します…」。
- **Open Question**: ログイン後の遷移先は正本に規定が無い。
  無料会員をマイページへ送る／送らないは **仕様として決めてから**変更する。
