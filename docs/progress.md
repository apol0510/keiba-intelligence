...
> 初版作成: 2026-07-20（基準コミット `1875508` = origin/main）
...
- main は自動取込 commit が日次で積まれており（最新 `1875508` = 2026-07-19 の JRA 結果検証）、**自動化パイプラインは稼働中**。
- **オープンな Pull Request は 0 件**（`gh pr list --state open` が空）。

> **重要な事実訂正**: 本タスクの当初想定では `feat/ki-umatan-f3` / `feat/ki-umatan-f3-remove-direction-badges` /
> `chore/umatan-archive-f3-5pt` を「進行中の未完了ブランチ」として扱う予定だったが、調査の結果
> **3 本とも（および `feat/ki-umatan-f3-mobile-layout` も）squash merge 済み**であることが確認された。
> 詳細は下記「Remaining」の「未整理ブランチ」を参照。
...
- **本ドキュメント基盤の Draft PR レビュー**（ブランチ `docs/autonomous-project-workflow`）。マージは未実施。

現時点で他に進行中のコード作業は確認されていない（オープン PR 0 件、未コミットの作業ツリー変更なし）。
...
- Phase 2: workflow 統合（8 → 5 へ削減）。**現状は 13 workflow に増えており、Phase 1 当時の前提と数が異なる**。再計画が必要。
...
4. Workflow Phase 2 を再計画する（現状 13 workflow を前提にした統合案を作る）。
...
2. **`BET_POINT_LOGIC.md` の検証表の数値はいつ時点のものか。**
   文書: 南関 217.1% / 公開的中 784 / ¥1,483,110、JRA 212.8% / 634 / ¥1,417,180
   2026-07-20 実測: 南関 214.8% / 902 / ¥1,673,170、JRA 212.9% / 744 / ¥1,647,970
...
4. Workflow Phase 2（統合）は依然として実施する方針か、それとも 13 workflow の現状維持で確定したのか。
5. lint / typecheck を導入しない判断は明示的になされたものか、単に未着手か。

## High-risk Operations Not Yet Executed

本タスクでは以下を **一切実行していない**。実行には個別の承認が必要。

- PR merge（本ブランチ `docs/autonomous-project-workflow` → `main`）
- main への直接 push / force push / reset / rebase / amend / revert / 履歴改変
- GitHub Actions の `workflow_dispatch` 実行 / `repository_dispatch` 送出
- Netlify 本番デプロイ / 本番環境変数・secret の変更
- Airtable・Netlify Blobs・共有リポジトリへの書込み
- SendGrid によるメール・メルマガ・アラートの送信
- `npm run verify:sync`（外部 API 呼び出しを伴うため未実行）
- `npm run build`（依存インストールを伴うため未実行）
- 既存ブランチの削除
...
- **Repository**: `keiba-intelligence`（作業は git worktree 上で実施。`astro-site/` が npm プロジェクトルート）
- **Branch**: `docs/autonomous-project-workflow`（`origin/main` から作成）
- **HEAD**: `1875508`（作業開始時点。`Auto-verify: 2026-07-19 JRA results from jv-link-cli` / 2026-07-19 10:53:03 +0000）
- **Origin**: `https://github.com/apol0510/keiba-intelligence`
- **Working tree**: 作業開始時 clean。本タスクの変更は `docs/spec.md` / `docs/progress.md` / `docs/decisions.md` / `CLAUDE.md` の 4 ファイルのみ（ソースコード変更なし）
