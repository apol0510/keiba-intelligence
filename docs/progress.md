...
| R-3 | 無料の印は 1 列に **指数ごとの印を合算**（新聞の総合印）。指数1本＝記者1人で 1位◎/2位○/3位▲/4〜10位△。同じ記号が重なる。**本命は分かってよい**（指数が一致すれば自然に印が集まる）。守るのは相手（△を相手5〜6頭より広く＝実測8〜12頭） | `attentionMarks.availableAxes` → `assignFreeMarks`（実在の指数のみ。1頭を特別扱いする処理なし。ランダム不使用） |
