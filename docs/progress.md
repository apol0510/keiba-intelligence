...
3. 🔴 **ライトとプレミアムの差が「会場」しかない（2026-08-30 発見）。**
   コードで実際に効いている差は `venueAccess` のみ:

   | tier | 印 | 南関の買い目 | JRAの買い目 | premium 限定 |
   |---|---|---|---|---|
   | light（venueAccess=`nankan`） | ✅ | ✅ | ❌ | ❌ |
   | premium（venueAccess=`all`） | ✅ | ✅ | ✅ | ✅（**使用箇所なし**） |

   `canSeePremiumExtras` / `showPremiumExtras` は計算され `get-session` でも返しているが、
   **どのページもこのフラグで出し分けていない**（`RaceNewspaper` / `RaceDayBoard` は
   Props の型に持つだけで参照していない）。

   一方 `src/lib/billing/plans.js` のプレミアム訴求には
   「メインレースの詳細レポート」「穴馬レポート」「優先メルマガ」が書かれており、
   **実装が存在しない機能を料金ページで謳っている状態**。

   → 仕様所有者の判断が必要。取り得る道は 2 つ:
     - (a) プレミアム限定コンテンツを実装して差を作る
     - (b) 訴求文言を実装の範囲（会場の差）に合わせて書き換える

   どちらも未着手。**課金導線を本番へ出す前に解消すること**（誇大表示になるため）。
