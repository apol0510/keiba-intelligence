...
🔴 **2026-09-06 時点で 17/17 PASS。** 判定の詳細は `docs/progress.md`
「2026-09-06 Stripe Test Mode E2E — ✅ 17/17 PASS」を参照。

| # | 区分 | 実施日 | 結果 | 備考 |
|---|---|---|---|---|
| 1 | A 認可 | 2026-09-06 | ✅ | 仕様所有者が実機確認 |
| 2 | A 認可 | 2026-09-06 | ✅ | 同上 |
| 3 | A 認可 | 2026-09-02 | ✅ | Checkout（`4242…`）→ `/mypage?checkout=success` |
| 4 | A 認可 | 2026-09-02 | ✅ | `checkout.session.completed` が 200 |
| 5 | A 認可 | 2026-09-06 | ✅ | 仕様所有者が実機確認 |
| 6 | A 認可 | 2026-09-04 | ✅ | 🔴 R-2（`connectLambda` 欠落）の修正後に `duplicate:true` を実測 |
| 7 | B リワード | 2026-09-02 | ✅ | `invoice.payment_succeeded` が 200 |
| 8 | B リワード | 2026-09-02 | ✅ | `RewardLedger` が 1 行だけ増えた |
| 9 | B リワード | 2026-09-03 | ✅ | `Points=100` / `PeriodMonths=1` / `SourceRef=in_…` / `OccurredAt` は支払い成功時刻 |
| 10 | B リワード | 2026-09-03 | ✅ | 再送しても行が増えない |
| 11 | B リワード | 2026-09-02 | ✅ | `ContractPriceYen=3980` / `jpy` / `price_1UAsLM…` |
| 12 | B リワード | 2026-09-03 | ✅ | `/mypage` の残高・今月の積み上げ |
| 13 | C 解約 | 2026-09-03 | ✅ | ポータルから解約 → 200 |
| 14 | C 解約 | 2026-09-03 | ✅ | 買い目が閉じ、印は見える |
| 15 | C 解約 | 2026-09-03 | ✅ | `CancelledAt` に解約日 |
| 16 | C 解約 | 2026-09-03 | ✅ | ポイントは残る |
| 17 | C 失敗 | 2026-09-05 | ✅ | 🔴 Test Clock で更新請求を失敗させて実測。`Status` のみ変更・`RewardLedger` 不増 |

🔴 **証跡の粒度に差がある。** #6 / #17 は `docs/progress.md` に実測値
（応答本文・Airtable の各列・台帳行数）を残してあるが、
**#1 / #2 / #5 は「仕様所有者が実機確認済み」という報告のみ**で、
画面ごとの観測値は記録されていない。

#### この E2E で見つかり、恒久修正した不具合

| 不具合 | 修正 |
|---|---|
| v1 Lambda で `connectLambda` を呼ばず、Blobs の冪等性が常に無効だった | PR #95 |
| **¥0 の請求（トライアル）で 100 pt が付与されていた** | PR #99 |
| 開催日の曜日が UTC ビルドで 1 日ずれていた | PR #100 |
