    ...
    venues.push({
      ...
      races: (rbData.races || []).map(r => ({
        ...
        horses: (r.horses || []).map(h => ({
          ...
          kinryo: h.weight != null ? String(h.weight) : '', computerIndex: toComputerIndex(h.computerIndex),
  ...
  for (const race of (rbData.races || [])) {
    for (const horse of (race.horses || [])) {
      ...
      const data = {
        ...
        computerIndex: toComputerIndex(horse.computerIndex)
      };
      ...
      if (pastRaces.length > 0) {
        ...
        data.recentRaces = pastRaces.slice(-5).reverse().map(pr => ({
          ...
          rank: pr.finish, finishStatus: pr.finishStatus || null, headCount: null,
          ...
          distanceGaiji: pr.distanceGaiji || null
        }));
      }
        ...
        .map(h => {
          ...
          if (horseDataMap && horseDataMap.has(lookupName)) {
            ...
            if (rbInfo && typeof rbInfo === 'object' && !Array.isArray(rbInfo)) {
              ...
              if (toComputerIndex(rbInfo.computerIndex) != null) horseObj.computerIndex = toComputerIndex(rbInfo.computerIndex);
            }
          }
