...
- AI予想の推奨馬に触れる場合は「AI推奨の軸馬」「AIが評価した○番」「AIが推奨した○番」等、役割を伏せた表現を使う
...
function formatResultData(data) {
  ...
  if (roles && roles.length > 0) {
    ...
    if (top3Roles.length > 0) {
      facts.push(`AI推奨馬（3着以内に入った馬のみ。ここに無い馬は言及禁止。役割名は伏せること）:`);
      top3Roles.forEach(r => {
        facts.push(`  ${r.horseNumber}番${r.horseName || ''}（AI推奨）`);
      });
    }
  }
  ...
  const openings = [
    ...
    `${raceLabel}を振り返ります。AIはレースごとに学び続け、結果を次回予想へフィードバック。止まることなく精度を高めています。`,
    `${raceLabel}の結果が揃いました。AIは勝ち馬の走りを分析し、次回予想に即座に反映。一戦ごとに賢くなっていきます。`,
...
【2文目以降】
上記の冒頭に続けて、3着以内に入った馬の活躍を中心に自然な振り返りを2〜3文で書いてください。
