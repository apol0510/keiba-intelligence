...
- `/Users/apolon/Projects/nankan-analytics/` へのアクセス・編集（2026-04-12解禁）
  ...
  - 作業前に必ずpwd・git remoteで現在プロジェクトを確認
...
- ✅ 基本は各Claudeウィンドウで**独立した1つのプロジェクト**を担当
- ✅ ウィンドウAでkeiba-intelligence、ウィンドウBでnankan-analytics
- ⚠️ 2026-04-12以降、nankan-analyticsとの連携作業のため横断アクセスは許可
  （ただし作業前にpwd/git remoteで現在地確認必須）
...
## 🤖 **Gemini AI 活用ロードマップ（2026-03-29策定）** 🤖

### **✅ 完了済み**

#### **AIチャットサポートウィジェット**
- **実装日**: 2026-03-29
- **技術**: Gemini 2.5 Flash + Netlify Functions
- **機能**: サービス・料金・予想についてのリアルタイムAI応答
- **UI**: クイック質問ボタン、フォローアップUI（解決確認・関連ページ誘導・お問い合わせ案内）

---

### **⏳ 今後の実装予定**

#### **Phase 1: AI予想解説（レース分析コメント）** ⭐ 優先度：高
- **概要**: 各レースの予想ページに「AIの見解」を自動生成
- **内容**: 「なぜこの馬が本命なのか」をFeature Importanceデータから自然言語で解説
- **効果**: ユーザーの理解度・納得感が向上、サービスの差別化
- **実装方法**: 予想データをGeminiに渡して解説文を生成、予想ページに表示
- **対象ファイル**: 予想ページ（prediction.astro等）、新規Netlify Function

#### **Phase 2: レース結果の自動振り返りコメント** ⭐ 優先度：高
- **概要**: 結果ページに「AI振り返り」を表示
- **内容**: 的中要因・外れた原因をデータから分析コメント生成
- **効果**: サービスの透明性アップ、ユーザー信頼度向上
- **実装方法**: 結果データをGeminiに渡して振り返り文を生成
- **対象ファイル**: 結果ページ（results.astro等）、新規Netlify Function

#### **Phase 3: メルマガ本文の自動生成** ⭐ 優先度：中
- **概要**: 週次レポートの文章をGeminiで自動生成
- **内容**: 的中実績データを渡して、読みやすいメルマガ本文を作成
- **効果**: メルマガ作成の工数削減、配信頻度向上
- **実装方法**: アーカイブデータ集計 → Geminiで文章生成 → SendGrid配信
- **対象ファイル**: 新規Netlify Function、既存のsend-broadcast.js拡張

#### **Phase 4: 馬券相談チャット（パーソナライズ）** ⭐ 優先度：中
- **概要**: 当日の予想データを元にした対話型相談
- **内容**: 「今日の大井で固いレースは？」「穴狙いならどのレース？」に回答
- **効果**: 有料会員限定機能として付加価値向上、解約率低下
- **実装方法**: 当日の予想JSONをGeminiに渡して対話応答
- **制限**: 有料会員（PRO）限定機能として実装

#### **Phase 5: 過去データQ&A** ⭐ 優先度：低
- **概要**: アーカイブデータを参照した質問応答
- **内容**: 「先月の的中率は？」「川崎の回収率は？」に回答
- **効果**: データ活用の利便性向上
- **実装方法**: アーカイブJSONを集計 → Geminiで自然言語応答
