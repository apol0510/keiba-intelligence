...
/**
 * 1 軸が印を出す頭数の目安。
 * 🔴 各軸は **上位 POOL_TARGET 頭**まで印を出す（4 位以下は △）。
 *    指数どうしは順位が似るため、△ を狭くすると
 *    合算しても △ が 4〜6 頭にしかならず、買い目の相手（5〜6 頭）を
 *    絞り込めてしまう（2026-08-29 実データで確認）。
 */
...
/**
 * 1 軸あたりの △ の頭数（4 位以下に付ける分）。
 * 🔴 軸の本数では変えない。各軸が独立に「上位 10 頭」まで印を出す。
 *    軸どうしの順位は似るので、ここを狭くすると合算しても △ が広がらない。
 */
export function downPerAxis(poolSize) {
  const n = Number.isInteger(poolSize) ? poolSize : 0;
  return clamp(Math.min(POOL_TARGET, n) - AXIS_MARKS.length, 0, POOL_MAX);
}
